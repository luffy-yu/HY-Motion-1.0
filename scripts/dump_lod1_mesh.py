...
def extract_skin_data(mesh_node, skeleton_nodes, joint_order):
    ...
    # Create mapping from MHR joint names to SMPL-H indices
    mhr_to_smplh_idx = {}
    for mhr_name, smplh_name in MHR_TO_SMPLH_MAPPING.items():
        if smplh_name in joint_order:
            mhr_to_smplh_idx[mhr_name] = joint_order.index(smplh_name)
    ...
    for c in range(cluster_count):
        ...
        if joint_name not in mhr_to_smplh_idx:
            # Try to find a match
            smplh_name = MHR_TO_SMPLH_MAPPING.get(joint_name)
            if smplh_name and smplh_name in joint_order:
                joint_idx = joint_order.index(smplh_name)
            else:
                continue
        else:
            joint_idx = mhr_to_smplh_idx[joint_name]
...
def extract_joint_positions(skeleton_nodes, joint_order):
    ...
    for mhr_name, smplh_name in MHR_TO_SMPLH_MAPPING.items():
        if smplh_name not in joint_order:
            continue
        if mhr_name not in skeleton_nodes:
            continue

        joint_idx = joint_order.index(smplh_name)
        node = skeleton_nodes[mhr_name]
...
def main():
    ...
    parser.add_argument(
        "--input", "-i",
        type=str,
        default="./assets/lod1.fbx",
        help="Path to lod1.fbx file"
    )
    parser.add_argument(
        "--output", "-o",
        type=str,
    ...
    try:
        ...
        skin_weights, skin_indices = extract_skin_data(mesh_node, skeleton_nodes, SMPLH_JOINT_ORDER)

        # Extract joint positions
        print("Extracting joint positions...")
        j_template = extract_joint_positions(skeleton_nodes, SMPLH_JOINT_ORDER)
